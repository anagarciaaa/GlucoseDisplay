...
sealed class BuiltInTransformer(context: SdkContext) : SdkTransformer(context) {
    /**
     * Built-in transformer which provides the dependencies value input directly to the output
     */
    class Identity(context: SdkContext) : BuiltInTransformer(context) {
        override fun onDependencyChange(
            timeStamp: Long,
            dependencies: Map<Dependency, Double>,
        ): Double {
            return dependencies.values.firstOrNull() ?: MISSING_VALUE
        }
    }
}
